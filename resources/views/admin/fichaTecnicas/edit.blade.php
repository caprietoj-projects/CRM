...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.fichaTecnica.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.ficha-tecnicas.update", [$fichaTecnica->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="descripcion">{{ trans('cruds.fichaTecnica.fields.descripcion') }}</label>
                <textarea class="form-control {{ $errors->has('descripcion') ? 'is-invalid' : '' }}" name="descripcion" id="descripcion" required>{{ old('descripcion', $fichaTecnica->descripcion) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('grupo') ? 'is-invalid' : '' }}" name="grupo_id" id="grupo_id" required>
                    @foreach($grupos as $id => $entry)
                        <option value="{{ $id }}" {{ (old('grupo_id') ? old('grupo_id') : $fichaTecnica->grupo->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('grupo'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('modelo') ? 'is-invalid' : '' }}" type="text" name="modelo" id="modelo" value="{{ old('modelo', $fichaTecnica->modelo) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('serial') ? 'is-invalid' : '' }}" type="text" name="serial" id="serial" value="{{ old('serial', $fichaTecnica->serial) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('color') ? 'is-invalid' : '' }}" type="text" name="color" id="color" value="{{ old('color', $fichaTecnica->color) }}" required>
                @if($errors->has('color'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('tipo') ? 'is-invalid' : '' }}" type="text" name="tipo" id="tipo" value="{{ old('tipo', $fichaTecnica->tipo) }}" required>
                @if($errors->has('tipo'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('ubicacion') ? 'is-invalid' : '' }}" name="ubicacion_id" id="ubicacion_id" required>
                    @foreach($ubicacions as $id => $entry)
                        <option value="{{ $id }}" {{ (old('ubicacion_id') ? old('ubicacion_id') : $fichaTecnica->ubicacion->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('encargado') ? 'is-invalid' : '' }}" name="encargado_id" id="encargado_id" required>
                    @foreach($encargados as $id => $entry)
                        <option value="{{ $id }}" {{ (old('encargado_id') ? old('encargado_id') : $fichaTecnica->encargado->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
